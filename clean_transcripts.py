...
for transcript in tqdm.tqdm(os.listdir("./transcripts")):
    if not os.path.isfile(
        os.path.join("transcripts", transcript.split(".")[0] + ".txt")
    ):
        ...
        try:
            ...
            content: str = " ".join([line.text for line in data]).replace("  ", " ")
        ...
        # We inspect such object and extract as much text as possible.
            ...
            for line in data["utterances"]:
                if "text" in line:
                    ...
                    raise ValueError(
                        f"Something with the subtitles {
                        transcript} is wrong"
                    )
            content = " ".join(alt_content).replace("  ", " ")
        finally:
            with open(
                os.path.join("clean_transcripts", transcript.split(".")[0] + ".txt"),
                "w",
                encoding="utf-8",
            ) as fp:
                fp.write(content)
