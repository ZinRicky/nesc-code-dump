...
for comment in tqdm(comments.itertuples(), total=comments.shape[0]):
    if not pd.isna(comment.reply_of):
        try:
            data[(comment.author, comments[comments['id'] == comment.reply_of].author.values[0])] += 1
        except:
            pass
    else:
        data[(comment.author, videos[videos['id'] == comment.video_id].author.values[0])] += 1

with open(os.path.join('polished_data', 'comment_graph.csv'), 'w', encoding='utf-8', newline='') as fp:
